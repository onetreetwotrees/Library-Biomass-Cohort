            ...
            for (int i = cohortData.Count - 1; i >= 0; i--) {
                ...
                if (reduction > 0) {
                    ...
                    if (reduction < cohort.Biomass) {
                        ...
                        // ReduceCohort calls PartialMortality, which adds dead wood biomass to woody pool on the forest floor in the amount by which live cohorts are reduced below. This needs to be a percent.
                        ReduceCohort(i, cohort, disturbance.CurrentSite, disturbance.Type, reductionPercent);
                        // cohort.ChangeBiomass(-reduction); reduces the LIVE biomass of a cohort by an amount caused by partial mortality. This is not the same as ReduceCohort above.
                        ...
                        // ReduceCohort is needed here as well (see above), to handle adding the dead wood from a partial mortality process to the forest floor woody pool, for when reductionPercent = 100%
